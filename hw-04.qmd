...
total_pop_change_20_23 <-total_pop_change_20_23 %>%
  mutate(
    change = `2023` - `2020`,
    NAME = str_remove(county, "^\\.") %>%                   # Remove leading dot
           str_remove(" County, Arizona$") %>%              # Remove ' County, Arizona' at the end
           str_trim()   
  )
```
...
arizona <- ggplot(az_counties_pop) +
  geom_sf(aes(fill = change))  +
    scale_fill_gradientn(
        ...
        breaks = c(0, 50000, 100000),                # Only show these values in the legend
        labels = c("0", "50,000", "100,000"),
